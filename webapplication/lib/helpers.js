...
/**
 * Returns the text compass direction for a heading in degrees
 *
 * Credit goes to
 ...
 *
 * @returns {string}
 *   Danish compas heading
 */
helpers.headingToText = function(heading) {
  // we might be called with an undef heading.
  if (!heading) {
    return '';
  }
  // We have 16 directions, 360/16 = 22.5 - we add .5 make sure we step up to
  // the next direction when we're right at the change.
  let val = parseInt((heading / 22.5)+.5);
  let arr = [
    'Nord',
    'Nordnordøst',
    'Nordøst',
...
helpers = _.merge(helpers, sharedHelpers);
